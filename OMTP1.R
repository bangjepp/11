...
Всем переменным заданы их реальные размерности
library(units)
...
library(readr)
BEF <- read_delim("D:/Загрузки/BEF.csv", 
                  ";", escape_double = FALSE, trim_ws = TRUE)
View(BEF)

BEF = BEF %>% filter(zone == "Southern taiga") %>%
  select(-age_group, -zone)

BEF =  BEF %>% mutate(AgeIndex = as.factor(age_group_indexes)) %>%
  mutate(AgeIndex = fct_recode(AgeIndex,Y = "I", 
                               SM = "II", 
                               EM = "III", M = "IV"))

BEF = BEF %>% mutate(Species =  case_when(
  Genum == "Quercus" ~ "Quercus robur",
  Genum == "Other hard deciduous" ~ "Acer platanoides",
  Genum == "Other soft deciduous" ~ "Platanus occidentalis",
  Genum == "Betula" ~ "Betula pendula"
))

BEF =  BEF %>% filter(!is.na(Species))

BEF$Species
BEF$AgeIndex

BEF = BEF %>% select(-Genum, -age_group_indexes)

citytrees2 = left_join(citytrees2, BEF, by = c("AgeIndex","Species"))

citytrees2$BA = (pi * citytrees2$dbh^2)/4

# Если мы не знаем форму дерева

citytrees2$Volume = citytrees2$BA*citytrees2$Ht*0.42 
units(citytrees2$BCEF) = as_units("kg/m^3")
citytrees2$BCEF = citytrees2$BCEF*1000 

citytrees2 = citytrees2 %>% mutate(biomass = Volume * BCEF*(1 + R)*0.5)
...
citytrees2_num = select_if(citytrees2, is.numeric)
v = sapply(citytrees2_num, function(x){var(x, na.rm = T)})
vnames  = names(v[v>0.01])
vnames = vnames[!is.na(vnames)]
citytrees2_num = citytrees2_num %>% select(all_of(vnames))
citytrees2_num = na.exclude(citytrees2_num)

citytrees2_pca = prcomp(citytrees2_num, center = TRUE, scale = TRUE)

ggbiplot(citytrees2_pca,ellipse=TRUE,  groups=citytrees2_pca$Species, circle = 1, 
         obs.scale = 3, labels = citytrees2_pca$Site)+theme_bw()+
  theme(aspect.ratio = 1)


#модель зависимости высоты и диаметра

a = data.frame(citytrees2)

library(tidyverse)
library(readr)
library(sf)

ggplot(data = a, aes(x = as.double(Ht), y=log(as.double(dbh, base = exp(1)))))+
  geom_point(aes(color=geology))+
